...
def parse_nested_dict(dict_nes, item_key):
    ...
    try:
        ...
        #print("Key match list for policesquad",key_match_list)
        if key_match_list:
...
def accumulator(files):
        ...
        for state_code, all_state_files in itertools.groupby(files, operator.itemgetter(0)):
            ...
            for school in [all_schools[1] for all_schools in all_state_files]:
                ...
                print("json file:",school_json_file)
                input_file = open(school_json_file, 'r')
                try:
                   json_input = json.load(input_file)
                   #if(school_json_file == "/media/durga/durga1/Home/Ct_Syncthg/data/ct/2018/ct33/2018/ct/-ct33/gstudio/gstudio_tools_logs/policesquad/0-policesquad.json"):
                ...
                for items in json_input:
                   try:
                       ...
                       if pandas.Series([tool_name]).isin(normal_tools)[0]:
                           ...
                           schools_tool_dict['time_spent_seconds'].append(None)
                           ...
                           #print("school dict",schools_tool_dict)
                           schools_tool_dict['createdat_start'].append(None)
                           schools_tool_dict['createdat_end'].append(parse_nested_dict(items, 'create'))
...
def fetch_log_level_data_tools(data_path, schools, state):
    ...
    #print("json files:", tools_json_files)
    for each in tools_json_files:
        print(('/'.join(each[1][1].split('/')[-5:])))
    list_grouped = [('/'.join(each[1][1].split('/')[-5:]), each) for each in tools_json_files]
    #print(sorted(list_grouped, key=lambda x: x[0]))
    k = itertools.groupby(sorted(list_grouped, key=lambda x: x[0]), key = lambda x: x[0])
    unique_file_iterator = []
    for key,grp in k:
        key_and_group = {key: list(grp)}  
        print(len(key_and_group[key]))
        if len(key_and_group[key]) == 2:
            unique_file_iterator.append(key_and_group[key][1])
        else:
            unique_file_iterator.append(key_and_group[key][0])
    #intr1 = map(operator.itemgetter(1), itertools.groupby(sorted(list_grouped, key=lambda x: x[0]), key = lambda x: x[0]))
    #print("intermetdiate result:", intr1)
    #for each in intr1:
    #	print next(each)
    ...
    #for each in unique_file_iterator:
    #    print(each)
    print(unique_file_iterator)
    tools_json_files_unique = [each[1] for each in unique_file_iterator]
    print("tool json files:",tools_json_files_unique)
...
def get_time_spent(df):
    ...
    def timespent(x):
        ...
        if not x[normal].empty:
           #print("max and min timespent:",max(x[normal]['createdat_end']),min(x[normal]['createdat_end']))
           #print("timespent:", ((max(x[normal]['createdat_end']) - min(x[normal]['createdat_end'])).seconds))
           x.loc[normal, ['time_spent']] = ((max(x[normal]['createdat_end']) - min(x[normal]['createdat_end'])).seconds/60)
           x.loc[normal, ['time_spent_seconds']] = (max(x[normal]['createdat_end']) - min(x[normal]['createdat_end'])).seconds
        if not x[ast_trek].empty:
           ...
           x.loc[ast_trek, ['time_spent']] = (ts_asttrek.seconds/60)
           x.loc[ast_trek, ['time_spent_seconds']] = ts_asttrek.seconds
        if not x[ast_moon].empty:
           ...
           x.loc[ast_moon, ['time_spent']] =  (ts_moon.seconds/60)
           x.loc[ast_trek, ['time_spent_seconds']] = ts_moon.seconds
        if not x[ice].empty:
           ts_ice =  reduce(lambda x,y: x + y + datetime.timedelta(0), x[ice]['createdat_end'] - x[ice]['createdat_start'])
           x.loc[ice, ['time_spent']] = (ts_ice.seconds/60)
           x.loc[ice, ['time_spent_seconds']] = ts_ice.seconds
        if not x[rationpatt].empty:
           ...
           x.loc[rationpatt, ['time_spent']] = (ts_ratio.seconds/60)
           x.loc[rationpatt, ['time_spent_seconds']] = ts_ratio.seconds
        if not x[polic].empty:
		    ...
		    #print("ts:",ts)
                    if isinstance(ts, int) or isinstance(ts, numpy.int64):
                       ...
                       hh_sec = int(ts.split(':')[0]) * 60 * 60
                       minut_sec = int(ts.split(':')[1]) * 60
                       sec_sec = float(ts.split(':')[2])
                    else:
                       print('New type for timespent in policesquad')
                       import pdb
                       pdb.set_trace()
                ...
                return hh_sec + minut_sec + sec_sec
            """
            def splittime(x):
               ts = x['time_spent']
               hh_minut = int(ts.split(':')[0]) * 60
            ...
            x.loc[polic, ['time_spent_seconds']] = x.loc[polic, ['time_spent']].apply(lambda x: get_second(x), axis=1)
            x.loc[polic, ['time_spent']] = x.loc[polic, ['time_spent']].apply(lambda x: get_minute(x), axis=1)
            #x.loc[polic, ['time_spent_seconds']] = x.loc[polic, ['time_spent']].apply(lambda x: get_minute(x), axis=1)
        return x

    df = df.groupby(['user_id', 'date_created', 'session_id','tool_name']).apply(lambda x: timespent(x))#.reset_index(level=None, drop=True)
    df.to_csv('timespent.csv')
    return df
...
def get_metrics_in_daterange(schools_df, date_range):
    ...
    schools_df.to_csv('schools_df.csv')
    try:
...
def get_clean_data(schools_df):
    ...
    def get_rid_of_spurious_dates(date_entry):
        ...
        if not pandas.isnull(date_entry):
            if '2019-0-' in str(date_entry) or '2020-0-' in str(date_entry):
